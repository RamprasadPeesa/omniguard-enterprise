/*
# Helper Functions and Critical Fixes

1. update_timestamp() - Auto-update timestamps
2. handle_new_user() - Auto-create user profile on auth signup
3. is_org_member() / is_org_admin() - Membership check helpers
4. generate_slug() - URL-safe slug generation
5. RLS DELETE policy fixes (were accidentally FOR UPDATE)
6. Expanded integrations provider CHECK
7. repositories.provider_id nullable
8. Open user_profiles read for team invites
9. Duplicate scan-queue trigger cleanup
...
-- Expand integrations provider CHECK
...
-- Open user_profiles read for team invites
...
-- Fix duplicate scan-queue triggers
